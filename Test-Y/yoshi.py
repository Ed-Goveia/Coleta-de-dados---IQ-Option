...
def mhi(opened, closed):
    ...
    cand = Iq.get_candles(goal, timeframe, 20,  time.time())

    ultimo = round(cand[0]['close'], 4)
    primeiro = round(cand[-1]['close'], 4)
    diferenca = abs( round( ( (ultimo - primeiro) / primeiro ) * 100, 3) )
    tendencia = "CALL" if ultimo < primeiro and diferenca > 0.02 else "PUT" if ultimo > primeiro and diferenca > 0.02 else "FALSE"
    ...
    if tipe_cicle !=3 or last_gale == 0:
        print(f'MT R:{r_a} G:{g_a} D:{doji_a}    TP R:{r_p} G:{g_p} D:{doji_a}     TR R:{r_y} G:{g_y} D:{doji_a}      MiC R:{r} G:{g} D:{doji}     Doj: {doji_lu}   ted: {tendencia}           ')
        print(f'----------------+-------------------+-------------------+------------------+----------------')
...
# doji operacional
    if doji == 0:
        ...
        if r_a >= 13: signal = 'call' if doji_lu == 0 else None
        if g_a >= 13: signal = 'put' if doji_lu == 0 else None  
        ...
        if doji_lu <= 1 :
            if g_q < 4 and r_q < 4:
                if r < 4 and g < 4:
                    ...
                    if r_a >= 11: 
                        if not r == 3 and r_last == 2 and r_t ==2: signal = 'call'
                    if g_a >= 11: 
                        if not g == 3 and g_last == 2 and g_t ==2: signal = 'put'
        if g == 5 or r == 5:
            ...
            if r_a - 1 > g_a or g_a - 1 > r_a:
                if g == 5:
                    if r_a < 10 and g_a < 10: signal = 'put' if doji_lu == 0 else None
                    if g_a == 10: signal = 'call' if doji_lu == 0 else 'put' if doji_lu == 1 else None
                    if g_a == 11: signal = 'put'  if doji_lu == 1 else 'call' if doji_lu == 0 else None
                    if g_a > 11 : signal = 'put'  if doji_lu <= 1 else None
                
                    else: signal = 'put' if doji_lu < 2 else None   
                if r== 5:
                    if r_a < 10 and g_a < 10: signal = 'call' if doji_lu == 0 else None
                    if r_a == 10: signal = 'put' if doji_lu == 0 else 'call' if doji_lu == 1 else None
                    if r_a == 11: signal = 'call' if doji_lu == 0 else 'put' if doji_lu == 1 else None
                    if r_a > 11: signal = 'call' if doji_lu <= 1 else None
                    
                    else: signal = 'call' if doji_lu < 2 else None    

    if doji_a >= 2: signal = None
    
    if doji_a <= 1:
        if r < 5 and g < 5: 
            if doji == 0:
                if doji_lu <= 1:
                    if r_a == 11 and g == 3 or r == 3: signal = 'put'
                    if g_a == 11 and g == 3 or r == 3: signal = 'call'
                            ...
                            if g_t < 3:
                                ...
                                if g == 4:
                                    if r_one == 1: 
                                        ...
                                        if r_a == 11: signal = 'call'
                                        if g_a == 11: signal = 'put' if doji_lu == 0 else None
                                        if doji_lu == 1:
                                            if r_a == 10: signal = 'call'
                                            if g_a == 10: signal = 'put'
                                        if doji_lu == 0:
                                            ...
                                            if r_a == 9: signal = 'call'
                                        if r_a - 1 == g_a or g_a - 1 == r_a : 
                                            if r_p > g_p or g_p > r_p:signal = 'call' # indecisão
                                        if r_a - 1 > g_a or g_a - 1 > r_a:
                                            if r_a < 9 and g_a < 9:
                                                if r_p == g_p: signal = 'call'
                                                if r_p < 9 and g_p < 9: 
                                ...
                                if g ==3:
                                    ...
                                    # Padrão - G R G G R --- complementado
                                    ...
                                    # Padrão - # G G R R G --- reduzido
                                    if r_t==2 and g_one == 1: 
                                        if r_a == 10: signal = 'put'
                                        if g_a == 9: signal = 'put'
                                        if r_a - 1 == g_a or g_a - 1 == r_a:
                                            if ((r_p - 1) - g_p) == 1: signal = 'put'
                                            if ((r_p - 1) - g_p) == 2: signal = 'call' 
                                            if g_p > r_p: signal = 'call'
                                            else: signal = 'put'
                                        if r_a - 1 > g_a or g_a - 1 > r_a:
                                            if r_p > g_p: signal = 'put'
                                            if g_p > g_p: signal = 'call'
                                            else: signal = 'call'
                                        else : signal = 'put'
                                    ...
                                    # Padrão - G R G R G --- Reduzido / complementado
                                    ...
                                    # Padrão - G R R G G --- Reduzido e complementado
                            ...
                            if r_t < 3:
                                ...
                                if r == 4:
                                    if g_one == 1: 
                                        ...
                                        if r_a == 11: signal = 'call' if doji_lu == 0 else None
                                        if g_a == 11: signal = 'put' 
                                        if doji_lu == 1:
                                            if r_a == 10: signal = 'call'
                                            if g_a == 10: signal = 'put'
                                ...
                                if r == 3:
                                    # Padrão - R R R G G --- complementado
                                    if g_last == 2: 
                                    ...
                                    # Padrão - R G R G R ---  Reduzido / complementado
                                    ...
                                    # Padrão -
                                    #  R R G R G - - - Reduzido   / complementado
                                    if g_t==2 and r_last==1 and g_one ==1: 
                                        if r_a == 10: signal = 'put'
                                        if g_a == 10: signal = 'call'
                                    ...
                                    # Padrão - R G R R G --- complementado
                                    ...
                                    # Padrão - R R G G R --- reformulado
                                    if g_t == 2 and r_one == 1: 
                                        if g_a == 10: signal = 'call'
                                        if r_a == 9: signal = 'call'
                                        if r_a - 1 == g_a or g_a - 1 == r_a:
                                            if ((g_p - 1) - r_p) == 1: signal = 'call'
                                            if ((g_p - 1) - r_p) == 2: signal = 'put' 
                                            if r_p > g_p: signal = 'put'
                                            else: signal = 'call'
                                        if r_a - 1 > g_a or g_a - 1 > r_a:
                                            if r_p > g_p: signal = 'put'
...
def ted():
    ...
    shit = False
    if tipe_cicle == 3:
        if g_a >= 10 and g_a <= 12: shit = True
        if r_a >= 10 and r_a <= 12: shit = True
        if doji_a == 1: shit = True
    else: shit = False
    return shit
try:
    while True:
        ...
        if is_time():
            ...
            if signal is None:
                while True:
                    print(f'Sinal: ',f'\033[35m{signal}\033[0m      {padrão} \n\n')
                    break
            ...
            if signal is not None:
                for i in range (einstein + 1):
                    ...
                    if res is not None:
                        if signal == 'put' : print(f'{padrão}       Ação: {signal}             Ciclo: {last_gale}           com: {buy_amount} {rex()}')
                        if signal == 'call': print(f'{padrão}       Ação: {signal}            Ciclo: {last_gale}           com: {buy_amount} {rex()}')
    ...
    print(f' Lucro atual: {total}                                                         ')
    print('\n Parando o Robô!                                                               ')
    input('\n Pressione \033[35m ENTER \033[0m para sair! ')
